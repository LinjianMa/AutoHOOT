...
from examples.cpd import cpd_graph, cpd_als, cpd_als_shared_exec, cpd_als_jax
...
def test_cpd_als_jax():
    for datatype in BACKEND_TYPES:
        ...
        outputs = cpd_als_jax(size, rank, 1, input_val)
        ...
        print(T.norm(outputs[0] - A_val))
        assert T.norm(outputs[0] - A_val) < 1e-5
        assert T.norm(outputs[1] - B_val) < 1e-5
        assert T.norm(outputs[2] - C_val) < 1e-5
test_cpd_als_jax()
