...
def mps_graph(num, rank, size=2):
    ...
    Each tensor is arranged as left leg, downward leg, right leg.
    The left one is arranged as downward leg, right leg and
    ...
    """
    A_left = ad.Variable(name='A0', shape=[size, rank])
    A_right = ad.Variable(name=f'A{num-1}', shape=[rank, size])

    A_middle_list = []
    for i in range(1, num - 1):
        node = ad.Variable(name=f'A{i}', shape=[rank, size, rank])
    ...
    prev_char = A_left.subscripts[-1]
    untracted_subs_list.append(A_left.subscripts[0])

    for node in A_middle_list:
        node.subscripts = f"{prev_char}{cg.getchar()}{cg.getchar()}"
        prev_char = node.subscripts[-1]
        untracted_subs_list.append(node.subscripts[1])
...
def mpo_graph(num, rank, size=2):
    ...
    Each tensor is arranged as left leg, upward leg, downward leg, right leg.
    The left one is arranged as upward leg, downward leg, right leg and
    ...
    2. The input nodes of the einsum node.
    """
    H_left = ad.Variable(name='H0', shape=[size, size, rank])
    H_right = ad.Variable(name=f'H{num-1}', shape=[rank, size, size])

    H_middle_list = []
    for i in range(1, num - 1):
        node = ad.Variable(name=f'H{i}', shape=[rank, size, size, rank])
    ...
    prev_char = H_left.subscripts[-1]
    up_subs_list.append(H_left.subscripts[0])
    down_subs_list.append(H_left.subscripts[1])

    for node in H_middle_list:
        node.subscripts = f"{prev_char}{cg.getchar()}{cg.getchar()}{cg.getchar()}"
        prev_char = node.subscripts[-1]
        up_subs_list.append(node.subscripts[1])
        down_subs_list.append(node.subscripts[2])
